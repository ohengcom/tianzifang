 ...
 * @returns {{ max, min, avg, peak, measured, stale, estimated, totalVisitors, peakHour, wd, confidenceCounts }}
 */
export function computeCrowdStats(samples, today) {
  ...
  const weekday = today ? new Date(`${today}T12:00:00+08:00`).getDay() : new Date(peak.ts).getDay();
  const wd = weekday === 0 ? 6 : weekday - 1;
  return { max, min, avg, totalVisitors, peak, peakHour, measured, stale, estimated, confidenceCounts, wd };
}
